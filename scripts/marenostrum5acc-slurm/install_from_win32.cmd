...
REM Download Blender 4.5.3 for Linux
echo Downloading Blender 4.5.3 for Linux...
powershell -Command "Invoke-WebRequest -Uri 'https://ftp.nluug.nl/pub/graphics/blender/release/Blender4.5/blender-4.5.3-linux-x64.tar.xz' -OutFile 'blender-4.5.3-linux-x64.tar.xz'"

REM Transfer Blender archive to MareNostrum5 cluster
echo Transferring Blender archive to MareNostrum5...
scp blender-4.5.3-linux-x64.tar.xz MareNostrum5:~/blender.tar.xz

REM Run the Linux installation script
echo Running installation script on MareNostrum5...
ssh MareNostrum5 "if [ -d ~/blender ] ; then rm -rf ~/blender ; fi ; cd ~/ ; tar -xf blender.tar.xz ; mv blender-4.5.3-linux-x64 ~/blender ; rm blender.tar.xz ;"
